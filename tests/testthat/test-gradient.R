test_that("gradient_hard matches finite differences", {
  skip_on_cran()

  grad <- gradient_hard(test_par_hard, test_X, test_y, test_qhat,
  ...
  for (j in seq_len(Dnoq)) {
    ...
    f_plus  <- objective_hard(par_plus, test_X, test_y, test_qhat, test_Hmat, test_A)$obj
    f_minus <- objective_hard(par_minus, test_X, test_y, test_qhat, test_Hmat, test_A)$obj
