test_that("Hessian matches finite differences", {
  ...
  H_mat <- hessian_hard(test_par_hard, test_X, test_y, test_qhat,
  ...
  for (j in seq_len(ncols)) {
    ...
    g_plus  <- gradient_hard(par_plus, test_X, test_y, test_qhat,
                             test_Hmat, test_A, tau_tmat = 0)
    g_minus <- gradient_hard(par_minus, test_X, test_y, test_qhat,
                             test_Hmat, test_A, tau_tmat = 0)
    fd_hess[, j] <- (as.numeric(g_plus) - as.numeric(g_minus)) / (2 * eps)
  }
  expect_equal(H_mat[, 1:ncols], fd_hess, tolerance = 1e-3)
})

test_that("combined grad_hess matches separate calls", {
  skip_on_cran()

  gh    <- grad_hess_hard(test_par_hard, test_X, test_y, test_qhat,
                          test_Hmat, test_A, lambda.diag = 0, tau_tmat = 0)
  g_sep <- gradient_hard(test_par_hard, test_X, test_y, test_qhat,
                         test_Hmat, test_A, tau_tmat = 0)
  H_sep <- hessian_hard(test_par_hard, test_X, test_y, test_qhat,
                        test_Hmat, test_A, lambda.diag = 0, tau_tmat = 0)
